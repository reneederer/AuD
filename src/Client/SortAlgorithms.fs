...
let init () : SortModel * Cmd<Msg> =
    let model =
        { Numbers = "9,5,1,2,3,7,8,6,4,0"
          Step = 0
          SortOrder = SortOrder.Desc
          Type = SortAlgorithmType.QuickSort
        }

    model, Cmd.none

let update (msg: Msg) (model: SortModel) : SortModel * Cmd<Msg> =
    match msg with
    | HeapChanged numbersStr ->
        { model with Numbers = numbersStr }, Cmd.none
    | Step step ->
...
let view (model: SortModel) (dispatch: Msg -> unit) =
    let rec getRow n xs =
        if xs = [] then []
        else
            let a, b =
    ...
    div
        ...
        [ div
          ...
          let nodeRows, steps =
              let numbers, steps =
                  heapify model.Numbers model.Step model.SortOrder
              numbers.Split(',')
              |> List.ofSeq
              |> getRow 0, steps
